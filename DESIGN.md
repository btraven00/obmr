# obflow — omnibenchmark monorepo helper
...
- Replacing `git submodules` for downstream consumers — `obflow` is a dev workflow tool.
- Hosting / CI integration beyond shelling out to `gh`.

## Layout
Given a benchmark file at `~/lab/<bench>/bench.yaml`, `obflow init` produces:

```
~/lab/<bench>/
  bench.yaml                 # canonical, remote-referenced
  bench.local.yaml           # generated, paths rewritten to local clones
  .obflow.lock                 # resolved modules, pinned SHAs, parent dir
...
- Parent dir is **not** auto-namespaced per benchmark; multiple benchmarks
  sharing modules will reuse the same clone. `obflow` detects an existing clone
  and verifies its remote URL matches before reusing.
- All tool state lives in `.obflow.lock` next to the YAML. No `~/.obflow/`,
  no per-module dotfiles inside the modules.

## Commands (v1 surface)
- `obflow init <bench.yaml> [--parent ../bench-modules]`
  Parse YAML, clone missing modules, write `.obflow.lock` and `bench.local.yaml`.
- `obflow status`
...
- `obflow render` is pure: it never modifies the canonical file in place.
...
1. **M1 — parse + clone**
   ...
   - `obflow init` clones modules into `--parent` (default `../bench-modules`).
   - Writes `.obflow.lock`. No render yet.
2. **M2 — fanout**
   - `obflow status`, `obflow foreach`, parallel execution, per-repo log prefix.
3. **M3 — branch ops**
   - `obflow checkout`, `obflow sync`. Safe handling of dirty trees.
4. **M4 — render**
   - `obflow render --mode local|remote`, `bench.local.yaml`.
5. **M5 — gh integration**
   - `obflow pr`, push + `gh pr create` per module, print URLs (and optionally
...
- Should `obflow init` refuse to run if `--parent` is inside a git repo
